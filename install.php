...
$handlerUrl = 'https://server.21vek.it/phone_time/handler.php';
$type = 'phone_data';
$propCode = 'PHONE_DATA';

$resultAddPropType = Test::call(
    'userfieldtype.add',
    [
        'USER_TYPE_ID' => $type,
        'HANDLER' => $handlerUrl,
        'TITLE' => 'Время, оператор и регион клиента',
        'DESCRIPTION' => 'Время, оператор и регион клиента '.$type,
        'OPTIONS' => [
            'height' => 70,
        ]
    ]
...
if ($resultAddPropType['result'] == true)
{
    ...
    $resultAddProp = Test::call(
        ...
        [
            'fields' => [
                ...
                'SHOW_IN_LIST' => 'Y',
                'EDIT_IN_LIST' => 'Y',
                'EDIT_FORM_LABEL' => 'Время, оператор и регион клиента',
                'LIST_COLUMN_LABEL' => 'Строка вывода времени клиента по номеру телефона',
                'SETTINGS' => []
            ]
        ]
    );
    ...
    $inst_app = Test::call(
        ...
        [
           'PLACEMENT' => 'CALL_CARD',
           'HANDLER' => 'https://server.21vek.it/phone_time/call_bind.php',
           'LANG_ALL' => [
              'ru' => [
                 'TITLE' => 'Время и регион клиента',
                 'DESCRIPTION' => 'Время и регион клиента',
              ],
           ],
        ]
    );
    if ($inst_app['error'])
    {
        echo $inst_app['error'] . ': ' . $inst_app['error_description'];
    }
    else
    {
        echo 'has been added successful <br>';
    }

    $resultAddPropDeal = Test::call(
        ...
        [
            'fields' => [
                ...
                'EDIT_IN_LIST' => 'Y',
                'EDIT_FORM_LABEL' => 'Время, оператор и регион клиента',
                'LIST_COLUMN_LABEL' => 'Время, оператор и регион клиента',
                'SETTINGS' => []
            ]
        ]
    );
    if ($resultAddPropDeal['error'])
    {
        echo $resultAddPropDeal['error'] . ': ' . $resultAddPropDeal['error_description'];
    }
    else
    {
        echo 'property ' . $propCode . ' has been added successful <br>';
    ...
    if ($resultAddPropCont['error'])
    {
        echo $resultAddPropCont['error'] . ': ' . $resultAddPropCont['error_description'];
    }
    else
    {
        echo 'property ' . $propCode . ' has been added successful <br>';
    ...
    if ($resultAddPropComp['error'])
    {
        echo $resultAddPropComp['error'] . ': ' . $resultAddPropComp['error_description'];
    }
    else
    {
        echo 'property ' . $propCode . ' has been added successful <br>';
