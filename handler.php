...
class Test extends CRest 
	{
		protected static function getSettingData()
		{
			...
			if(file_exists(__DIR__ . '/domain/'.$domain.'_settings.json'))
			{
          	...
          	$f=fopen(__DIR__."/upload/wlog.txt", "a+");
            fwrite($f, print_r(array('return' => $return),1)."\n========\n");
            fclose($f);
			return $return;
		}
       ...
       /* protected static $dataExt = [];
         ...
         {
         	$domain = $_REQUEST['DOMAIN'];
            $return = static::expandData(__DIR__ . '/domain/'.$domain.'_settings.json'));
            if(is_array($return))
            {
         ...
         {
            $domain = $_REQUEST['DOMAIN'];
			return  (boolean)file_put_contents(__DIR__ . '/domain/'.$domain.'_settings.json', static::$dataExt = $data);
         }*/
...
if ($_REQUEST['PLACEMENT'] == 'USERFIELD_TYPE'):
    ...
    } if ($placementOptions['ENTITY_ID'] == 'CRM_DEAL' && $placementOptions['ENTITY_VALUE_ID'] > 0)
    {
        $result = Test::call(
            'crm.deal.get',
            [
                'ID' => intVal($placementOptions['ENTITY_VALUE_ID'])
            ]
        );
        
        if(!empty($result['result']['CONTACT_ID']))
        {
            $get_contact = Test::call(
                'crm.contact.get',
                [
                    'ID' => intVal($result['result']['CONTACT_ID'])
                ]
            );
            
            $phone = trim($get_contact['result']['PHONE'][0]['VALUE']);
            $phone_def = substr($phone, 0, 4);
            $phone_def_2 = substr($phone, 0, 2);
            $phone_def_3 = substr($phone, 0, 3);
            $phone_code = substr($phone, 0, 7);
        ...
        } elseif (!empty($result['result']['COMPANY_ID']))
        {
            $get_company = Test::call(
                'crm.company.get',
                [
                    'ID' => intVal($result['result']['COMPANY_ID'])
                ]
            );
            
            $phone = trim($get_company['result']['PHONE'][0]['VALUE']);
            $phone_def = substr($phone, 0, 4);
            $phone_def_2 = substr($phone, 0, 2);
            $phone_def_3 = substr($phone, 0, 3);
            $phone_code = substr($phone, 0, 7);
    ...
    } if ($placementOptions['ENTITY_ID'] == 'CRM_CONTACT' && $placementOptions['ENTITY_VALUE_ID'] > 0)
    {
        $result = Test::call(
            'crm.contact.list',
            [
    ...
    } if ($placementOptions['ENTITY_ID'] == 'CRM_COMPANY' && $placementOptions['ENTITY_VALUE_ID'] > 0)
    {
        $result = Test::call(
            'crm.company.list',
            [
